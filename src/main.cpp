...
uint256 hashGenesisBlock("0xf660027aa05ca27947ed04ea716562798a7f7bdb67bb50fa02d845bd335438f3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Taxicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Taxicoin Signed Message:\n";
...
{
    // Taxicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Check whether or not the given block is destined to be a super bock
bool static IsSuperBlock(int nHeight) {
    int64 blockNum;
    int64 maxBlock;
    // Block belongs to phase I (50% mined coins)
    if (nHeight < 233760) {
        // 
        maxBlock = 58440;
        blockNum = nHeight % maxBlock;
    }
    else {
        int offset = nHeight - 233760;
        maxBlock = 350000;
        blockNum = offset % maxBlock;
    }

    if (blockNum <= 13440 || blockNum >= maxBlock - 13440)
            return (blockNum % 320 == 0);

    return false;
}

// Calculate the value of the super block
double static GetSuperBlockMultiplier(int nHeight) {
    int blockNum;
    int origBlockNum;
    if (nHeight < 233760) {
        blockNum = nHeight % 58440;
        origBlockNum = blockNum;
        if (blockNum >= 45000)
            blockNum -= 45000;
    }
    else {
        int offset = nHeight - 233760;
        blockNum = offset % 350000;
        origBlockNum = blockNum;
        if (blockNum >= 336560)
            blockNum -= 336560;
    }

    double percent = (double)blockNum / 13440.0;
    int sBlock = (int)(42.0 * percent);

    double multiplier = 1.0;
    // Calculate the multiplier: (1/100)(superBlock)^2
    if (origBlockNum <= 13440)
        multiplier = ((1.0/100) * (pow((double)(42-sBlock), 2.0))) + 1.0;
    else
        multiplier = ((1.0/100) * (pow((double)(sBlock), 2.0))) + 1.0;
...
{
    int64 nSubsidy;
    int BlockValueI = 58440;

    // Check if the block is in phase I
    if (nHeight < 233760) {
        if      (nHeight < BlockValueI * 1) { nSubsidy = 250 * COIN; }
        else if (nHeight < BlockValueI * 2) { nSubsidy = 125 * COIN; }
        else if (nHeight < BlockValueI * 3) { nSubsidy = 65  * COIN; }
        else if (nHeight < BlockValueI * 4) { nSubsidy = 35  * COIN; }
    }
    ...
    // else in phase II
        ...
        // Reduced by 25% every 350,000 blocks (~1 year)
        nSubsidy = nSubsidy * (pow(3.0, (double)(nHeight / 350000)) /  pow(4.0, (double)(nHeight / 350000)));
    }

    double multiplier = 1.0;
    // Check it its a super block
    // if so, get the super block multiplier
    if (IsSuperBlock(nHeight))
        multiplier = GetSuperBlockMultiplier(nHeight);

    return ((int64)(nSubsidy * multiplier)) + nFees;
}

static const int64 nTargetTimespan = 6 * 60 * 60; // Taxicoin: 6 hours
static const int64 nTargetSpacing = 90; // Taxicoin: 90 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Taxicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Taxicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Taxicoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x05f0b616d87a536728bd7808dab21c4b8c55a45261d6d4aaacc72022280451bd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "19/Feb/2014 - Sochi Olympics: Canada hockey team beats Latvia 2-1";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1392846434;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1773648;

        if (fTestNet)
        {
            block.nTime    = 1392846434;
            block.nNonce   = 1231376084;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x0b98dc03fc30628b2fc56c61b3495e9657bdf3a67dc7dc0f428ab8b0983f0f2d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Taxicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TaxicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TaxicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TaxicoinMiner(CWallet *pwallet)
{
    printf("TaxicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Taxicoin-miner");
    ...
    try { loop {
        ...
        printf("Running TaxicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TaxicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TaxicoinMiner, pwallet));
}
